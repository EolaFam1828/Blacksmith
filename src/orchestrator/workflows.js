...
export const generateStepsForCommand = (command, task, classification) => {
  switch (command) {
    case "refactor":
      return [
        makeStep("Research best practices", "gemini-2.0-pro", `Research best practices for: ${task}`, "research", { department: "research" }),
        ...
        makeStep("Final review", "gemini-2.0-pro", `Review the completed refactor for: ${task}`, "review")
      ];
    ...
    case "build":
      if (classification.complexity === "high") {
        return [
          ...
          makeStep("Review", "gemini-2.0-pro", `Review the completed implementation for: ${task}`, "review")
        ];
      }
      return [];
    ...
    case "review":
      return [
        makeStep("Spec compliance check", "gemini-2.0-flash", `Check spec compliance for: ${task}. List deviations only.`, "spec_check", { department: "engineering" }),
        makeStep("Quality review", "gemini-2.0-pro", `Perform a thorough quality review for: ${task}`, "quality_review", { department: "engineering" })
      ];
    ...
    case "deploy":
      return [
        makeStep("Pre-deploy checks", "gemini-2.0-flash", `Run pre-deployment checks for: ${task}`, "checks", { department: "infrastructure" }),
